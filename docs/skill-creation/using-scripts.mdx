...
<Note>
The same relative-path convention works in `references/*.md` files. Paths are always relative to the **skill directory root**, not the file's own location.
</Note>
...
Use a format with explicit field delimiters — JSON, CSV, TSV — rather than free-form text or column-aligned tables. Column-aligned tables rely on whitespace to separate fields, which becomes ambiguous with missing values or multi-word fields. Delimited formats have unambiguous field boundaries that both LLMs and standard tools (`jq`, `cut`, `awk`) can process reliably.

```
# Field boundaries depend on whitespace alignment
NAME          STATUS    CREATED
my-service    running   2025-01-15

# Field boundaries are explicit
...
- **Predictable output size.** Many agent harnesses automatically truncate tool output beyond a threshold (e.g., 10-30K characters), potentially losing critical information from the middle. If your script might produce large output, default to a summary or a reasonable limit, and support flags like `--limit` or `--verbose` so the agent can request more detail when needed.
